    ...
    public void run(String... strings) throws Exception {
        ...
        for (int month = 4; month <= 6; ++month)
            for (int day = 1; day <= 31; ++day) {//之后删掉不存在的日期
                if(friendshipService.needStop())
                    break;

                String date = "20160" + month + (day < 10 ? ("0" + day) : day);
                if(friendshipService.isRecorded(date)){
                ...
                for(TransactionEntity transactionEntity : transactionEntityList) {
                    ...
                    if(preTime == null || transactionEntity.getTranstime().getMinutes() != preTime.getMinutes()) {
                        ...
                        logger.info("Now: " + date + "|" + transactionEntity.getTranstime() + ", mc: " + minuteCount + ", Yixin: " + queues.get(terminalBelong.get(transactionEntity.getTermid())).size());
                        minuteCount = 0;
                    }
                ...
                friendshipService.recorded(date);
